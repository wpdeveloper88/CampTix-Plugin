  ...
  protected $options;
	...
	function save_ticket_post( $post_id ) {
		...
		if ( ! isset( $_POST['action'] ) || 'editpost' != $_POST['action'] )
			return;
		$meta_values = get_post_meta($post_id, 'ticket-to-prod', true);
		// Save the WPSC product during the ticket save
		$post_ticket = get_post($post_id); 
		$title = $post_ticket->post_title;
		$post_author = $post_ticket->post_author;
		$post_name = $post_ticket->post_name;
		$status = $post_ticket->status;
		$post_excerpt = $post_ticket->post_excerpt;
		
		if(isset($meta_values) && $meta_values>0){
			$post_array = array(
